<script lang='ts'>
	...
	export let filterBackAdjustment: TAdjustment | undefined = undefined
	export let filterBackMinorCount: number | undefined = undefined

	function getAbouts(as: TAbout[], adjustment: TAdjustment | undefined, minorCount: number | undefined) {
		let minorsCounted = 0

		return as
			.reduce<TAbout[]>((results, about) => {
				if (!(!about.hidden || (typeof about.hidden === 'function' && !about.hidden()))) return results

				const nextAbout = {
					...about,
					minors: about.minors
						.reduce<typeof about.minors>((minorResults, minor) => {

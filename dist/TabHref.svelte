...
export let tabHrefs = [];
export let basePath = void 0;
export let storage = "session";
export let saveKey = null;
...
function getSlug(tabItem) {
  return !tabItem ? null : tabItem.href ?? `${tabPrefix}${ReplaceAll(" ", "", tabItem.key)}`;
}
$: visibleItems = tabHrefs.filter((tabItem) => !tabItem.hidden).map((tabItem) => ({
  ...tabItem,
...
function getTabSetting() {
  const tabSetting = storage === null || !browser ? null : storage === "session" ? sessionStorage.getItem(saveKey ?? `Tab${noDigitsBasePath}`) : localStorage.getItem(saveKey ?? `Tab${noDigitsBasePath}`);
  if (tabSetting?.startsWith("Tab")) return tabSetting;
  return null;
}
function setTabSetting(href) {
  if (consoleLogs) {
    console.info("setTabSetting", href);
  }
  if (storage && browser && href && href.toString().startsWith("Tab") && href !== basePath) {
    return storage === "session" ? sessionStorage.setItem(saveKey ?? `Tab${noDigitsBasePath}`, href) : localStorage.setItem(saveKey ?? `Tab${noDigitsBasePath}`, href);
  }
}
function getIDSetting() {
  return noIDRemember || !storage ? void 0 : storage === "session" ? sessionStorage.getItem(saveKey ?? `IDTab${noDigitsBasePath}`) : localStorage.getItem(saveKey ?? `IDTab${noDigitsBasePath}`);
}
function setIDSetting() {
  if (storage && basePathDigits && !noIDRemember) {
    return storage === "session" ? sessionStorage.setItem(saveKey ?? `IDTab${noDigitsBasePath}`, basePathDigits) : localStorage.setItem(saveKey ?? `IDTab${noDigitsBasePath}`, basePathDigits);
  }
}
async function mountAction() {
  if (browser && pathAnalyzer && storage && pathAnalyzer?.basePath.endsWith(
    `/${basePath}`
  ) && (pathAnalyzer.activePageComponents?.length ?? 0) < 3) {
    await tick();
    ...
    if (previousID) {
      ...
      if (consoleLogs) {
        console.info(
          ...
          newTab,
          "slug",
          pathAnalyzer?.activePageSlug,
          "URL",
          pathAnalyzer?.open(newTab ?? "", false),
          pathAnalyzer.basePathReplaced(
            `${noDigitsBasePath}${previousID}`,

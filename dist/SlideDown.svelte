  ...
  if (show) {
    ...
    switch (e.key) {
      case "Escape":
        ...
        break;
      case "ArrowDown":
        highlightedIndex = visibleDDActions.findIndex((visibleDDAction, idx) => idx > highlightedIndex && !visibleDDAction.disabled && !!visibleDDAction.action);
        if (highlightedIndex < 0) {
          highlightedIndex = visibleDDActions.findIndex((visibleDDAction) => !visibleDDAction.disabled && !!visibleDDAction.action);
        }
        e.preventDefault();
        break;
      case "ArrowUp":
        highlightedIndex = visibleDDActions.reduce((result, visibleDDAction, idx) => idx < highlightedIndex && idx > result && !visibleDDAction.disabled && !!visibleDDAction.action ? idx : result, -1);
        if (highlightedIndex < 0) {
          highlightedIndex = visibleDDActions.reduce((result, visibleDDAction, idx) => idx > result && !visibleDDAction.disabled && !!visibleDDAction.action ? idx : result, -1);
        }
        e.preventDefault();
        break;
      case "Enter":
...
const performAction = (e, ddAction) => {
...
$: !!show && scrollToActive();
const ddStyle = !maxHeight ? void 0 : `max-height: ${maxHeight};`;

<script>import { DeepEqual, SearchRows } from "@solidbasisventures/intelliwaketsfoundation";
...
export let use = [];
export let possibles;
export let selected = [];
export let selectedIDs = void 0;
export let created = [];
export let existing = [];
...
export let createValid = void 0;
export let displayValue = (item) => item.name ?? item.id ?? "";
export let idValue = (item) => item.id ?? displayValue(item);
export let keyValue = (item) => idValue(item);
export let inputValue = (item) => idValue(item);
...
async function resizeDD() {
  await tick();
  resizeTS = (/* @__PURE__ */ new Date()).valueOf();
}
...
$: listItems = SearchRows(possibles.filter((poss) => !selected.some((sel) => DeepEqual(idValue(sel), idValue(poss)))), searchValue).map((item) => ({
  display: displayValue(item),
  id: idValue(item),
  item
}));
function setSelectedIDs(sels) {
...
function setSelected(selIDs, poss) {

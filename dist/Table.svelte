<script>import { ShowMessageBox } from "./Definitions";
import {
  CleanNumber,
  DeepEqual,
  initialSortColumn,
...
let internalSelectedIDs = (!rememberKey || !browser ? selectedIDs : JSONParse(window.localStorage.getItem(`${rememberKey}SelectedIDs`))) ?? [];
$: internalSelectedIDs = (!rememberKey || !browser ? selectedIDs : JSONParse(window.localStorage.getItem(`${rememberKey}SelectedIDs`))) ?? [];
...
const doToggleSelected = (e, id) => {
  if (internalSelectedIDs) {
    if (e.shiftKey && lastSelectedID !== null) {
      internalSelectedIDs = [...internalSelectedIDs, ...SelectBetweenIDs(rows.map((datum) => datum[selectColumn]), lastSelectedID, id)].reduce((results, selectedID) => results.includes(selectedID) ? results : [...results, selectedID], []);
    } else {
...
$: setSelectedIDs(rows);
$: pageCount = CleanNumber(validRowCount) > 0 ? Math.floor((CleanNumber(validRowCount) + (CleanNumber(limit ?? 200) - 1)) / CleanNumber(limit ?? 200)) : 0;

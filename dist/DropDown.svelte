<script>import DropDownControl from "./DropDownControl.svelte";
...
export let buttonClass = void 0;
export let controlClass = void 0;
...
const refs = [];
let keystrokeSelectIndex = null;
let visibleDDActions;
$: visibleDDActions = ddActions.filter((ddAction) => !ddAction.hidden).map((ddAction, idx) => ({
  ...ddAction,
  active: keystrokeSelectIndex === null ? ddAction.active : idx === keystrokeSelectIndex
})).reduce((results, ddAction) => {
  if (ddAction.divider) {
...
function setKeystrokeSelectIndex(idx) {
...
const performAlternateAction = (ddAction) => {

...
export class PathAnalyzer {
    ...
    get activePageSlug() {
        if (this.base === '')
            return this.activePageComponents[0] ?? null;
        return this.activePageComponents[1] ?? null;
    }
    get activePageSlugs() {
        if (this.base === '')
            return BuildPath(...this.activePageComponents ?? null);
    ...
    basePathReplaced(newBaseComponent, slug) {
            ...
            if (component !== '~') {
                ...
                if (selectedIndex >= activePageIndex) {
                    ...
                    link = activePageIndex > 0 && activePageIndex <= idx && component === activePage ?
                        this.pathName :
                        BuildPath('/', ...pathNameComponents.slice(0, idx).filter(item => item !== '~'), successors.length ? '~' : '', ...successors, component);

...
export let noShowValue = !show ? show : !show;
...
export let noSystemCloseActions = false;
export let onOK = () => {
};
export let onCancel = () => {
};
...
let x = 0;
let y = 0;
async function changeIsShowing(isShow, isNoShowValue) {
  await tick();
  if (!isShowing) {
    x = 0;
    y = 0;
...
<dialog class='bg-white drop-shadow-xl shadow-xl rounded-lg overflow-hidden w-full p-0 max-w-full
			   dark:bg-slate-600 dark:text-white {clazz}'
				...
				on:click={() => {
			if (!isDialogMouseDown && !noSystemCloseActions) cancelAction()
        }}
